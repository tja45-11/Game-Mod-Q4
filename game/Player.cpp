...
void idInventory::GivePowerUp( idPlayer *player, int powerup, int msec ) {
	if (powerup == 11) {
		...
		case 0:
			anemoUp += 1;
			player->hud->SetStateInt("anemoUp", anemoUp);
			break;
		case 1:
			pyroUp += 1;
			player->hud->SetStateInt("pyroUp", pyroUp);
			break;
		case 2:
			cryoUp += 1;
			player->hud->SetStateInt("cryoUp", cryoUp);
			break;
		case 3:
			hydroUp += 1;
			player->hud->SetStateInt("hydroUp", hydroUp);
			break;
		case 4:
			geoUp += 1;
			player->hud->SetStateInt("geoUp", geoUp);
			break;
		}
	}
	else {
